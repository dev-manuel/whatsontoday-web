...
case class EventDetail(id: Option[Int], name: String, from: Timestamp,
                       to: Timestamp, creator: Login, categories: List[Category],
...
object EventDetail {
  ...
  implicit class EventDetailQuery(q: Query[EventTable, Event, Seq]) {
    def detailed(implicit ec: ExecutionContext) = {
      val s = q.join(LoginTable.login).on(_.creatorId === _.id).join(LocationTable.location).on(_._1.locationId === _.id)
