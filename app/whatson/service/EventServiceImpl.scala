  ...
  def insertCSV(file: File, login: Login): Future[List[EventDetail]] = {
    ...
    events.mapFutureSync { case map =>
      ...
      List("Name","From","To","Description", "ShortDescription", "Categories",
           "LocationName","Country","City","Street", "OrganizerId","PriceMin","PriceMax").map(default.getOrElse(_,None))
        .foldRight(Some(List()) : Option[List[String]]) {
          ...
          case Some(l) => {
            ...
            val loc = Location(None,l(6),None,None,l(7),l(8),l(9),None,None,None,None,None)
            ...
            db.run(categoryQuery.result).zip(locationQuery).flatMap { case (categories,location) =>
              val event = Event(None, l(0),
                                ...
                                l(3), l(4),
                                login.id, location.id.getOrElse(-1), Some(l(10).toInt), l(11).toBigDecimal,l(12).toBigDecimal)
